...
class Requests:
    ...
    def request(self, reqfunc, url, params, files=None):
        ...
        header = {"user-agent": "pella's apps"}
        
        if files:
            response = reqfunc(url, params, headers=header, files=files)
        else:
            response = reqfunc(url, params, headers=header)
        ...
        if response.status_code != 200:
            ...
            if response.json()["description"] == "Bad Request: user not found":
                return response
            
            if response.json()["description"] == "Bad Request: chat not found":
                return response
