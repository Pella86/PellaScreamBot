...
class IsGroupMemberList(ChatMemberList):
    ...
    def get_status_api(self, group_id, user_id):
        print("Requesting Chat member", group_id, user_id)
        
        chat_member_res = tg_requests.getChatMemeber(group_id, user_id)
        
        chat_member_json = chat_member_res.json()
        
        if "error_code" in chat_member_json:
            print("IsGroupMemberList: error_code", chat_member_json["description"])
            return False
        
        chat_member = tg_obj.ChatMember(chat_member_json["result"])
        
        print("status:", chat_member.status)
